...
def augment_data_inplace(data: np.ndarray) -> np.ndarray:
    """เพิ่ม noise ให้ข้อมูลโดยไม่เพิ่มจำนวนข้อมูล"""
...
def prepare_train_test_data(num_augmentations: int = 1):
    """เตรียมข้อมูลสำหรับ train และ test พร้อมการเพิ่ม augmentation"""
    # 1. โหลดและแยกข้อมูล
    ...
    # 3. เพิ่ม augmentation ด้วย noise
    print(f"\nApplying noise augmentation...")

    # เพิ่ม noise แต่ไม่เพิ่มข้อมูล
    normal_data_aug = add_gaussian_noise(normal_data_balanced)
    abnormal_data_aug = add_gaussian_noise(abnormal_data_balanced)

    print_data_distribution("After noise augmentation", normal_labels_balanced, abnormal_labels_balanced)

    # 4. สร้าง windows
    X_normal, y_normal = prepare_windows(normal_data_aug, normal_labels_balanced)
    X_abnormal, y_abnormal = prepare_windows(abnormal_data_aug, abnormal_labels_balanced)
